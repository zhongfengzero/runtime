...
mono_arch_patch_plt_entry (guint8 *code, gpointer *got, host_mgreg_t *regs, guint8 *addr)
{
	...
	g_async_safe_printf ("\n");
	g_async_safe_printf ("mono_arch_patch_plt_entry begin\n");
	g_async_safe_printf ("code addr: %p\n", code);
	g_async_safe_printf ("ins0: %08x\n", ((guint32*)code) [0]);
	g_async_safe_printf ("ins1: %08x\n", ((guint32*)code) [1]);
	g_async_safe_printf ("ins2: %08x\n", ((guint32*)code) [2]);

	/* adrp */
	ins = ((guint32*)code) [0];
	g_async_safe_printf ("ins: %08x\n", ins);
	if (((ins >> 24) & 0x1f) == 0x10) {
    		g_async_safe_printf("ADRP instruction detected\n");
		// adrp x16, address
		// add x16, x16, offset
		disp = (((ins >> 5) & 0x7ffff) << 2) | ((ins >> 29) & 0x3);
    		g_async_safe_printf("disp: %08x\n", disp);
		/* FIXME: disp is signed */
		g_assert ((disp >> 20) == 0);

		slot_addr = ((guint64)code + (disp << 12)) & ~0xfff;
		g_async_safe_printf("slot_addr: %llx\n", slot_addr);

		/* add x16, x16, :lo12:got */
		ins = ((guint32*)code) [1];
        	g_async_safe_printf ("ins[1]: %08x\n", ins);
		g_assert (((ins >> 22) & 0x3) == 0);
		slot_addr += (ins >> 10) & 0xfff;
        	g_async_safe_printf ("slot_addr (adrp): %llx\n", slot_addr);
	} else if (((ins >> 24) & 0x1f) == 0x15) {
		g_async_safe_printf("ADR instruction detected\n");
		// nop
		// adr x16, address
		ins = ((guint32*)code) [1];
        	g_async_safe_printf ("ins[1]: %08x\n", ins);
		...
		if (disp & (1 << 20)) {
    		...
    		g_async_safe_printf("disp fix: %08x\n", disp);
		/* FIXME: disp is signed */
		g_assert ((disp >> 20) == 0);
   		slot_addr = (guint64)((char*)code + disp);
        	g_async_safe_printf ("slot_addr (adr): %llx\n", slot_addr);
	} else {
	...
	/* ldr x16, [x16, <offset>] */
	ins = ((guint32*)code) [2];
	g_async_safe_printf ("ins[2]: %08x\n", ins);
	g_assert (((ins >> 24) & 0x3f) == 0x39);
	slot_addr += ((ins >> 10) & 0xfff) * 8;
	g_async_safe_printf ("slot_addr (ldr): %llx\n", slot_addr);

	g_assert (*(guint64*)slot_addr);
	*(gpointer*)slot_addr = addr;

	g_async_safe_printf ("mono_arch_patch_plt_entry end\n");
}
